...
IUSE="alsa bios dell esd gnutls havekernel +modules ncurses pulseaudio +sdl test vde"
RESTRICT="test"
...
pkg_setup() {
	...
	if use dell && ! use bios; then
		eerror "The 'dell' use flag required 'bios' use flag to be enabled too."
		die "Please enable the 'bios' use flag"
	fi
...
src_prepare() {
	...
	if use dell; then
		...
		sed -i 's/CFLAGS = -m32/CFLAGS = -m32 -DOEMID_ADDR=0xFE076 "-DOEMID_STR=\\"Dell System\\""/' bios/Makefile
	fi
